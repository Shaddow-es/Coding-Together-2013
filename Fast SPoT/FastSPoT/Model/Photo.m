...
+ (Photo *) photoFromFlickr:(NSDictionary *)flickrData
{
    ...
    // Si no somos iPad recupera la imagen en tamaño grande, si es iPad la original (más grande)
    FlickrPhotoFormat photoFormat = ([[UIDevice currentDevice] userInterfaceIdiom]
                                == UIUserInterfaceIdiomPad) ? FlickrPhotoFormatOriginal : FlickrPhotoFormatLarge;
    photo.imageURL = [FlickrFetcher urlForPhoto:flickrData format:photoFormat];
    return photo;
}
